...
_GUIDANCE = {
    ...
    "capabilities": (
        ...
        "script — the user saves that once, after Aspen has checked it over; and it "
        "can ping them when a run ends, if they ask for that. That last message says "
        "where the output landed — Aspen's own staging area, not their calculations "
        "tree, since it writes nothing there — and hands over the copy command; "
        "Aspen can read those results back to answer questions about them, but "
        "copying them anywhere is theirs to run. None "
