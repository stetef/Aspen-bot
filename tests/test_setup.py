...
@pytest.fixture
def people(sut, env):
    """Sam (admin), Arun with his own root, and a PI who has neither."""
    ...
    return {"tmp": env.calcs, "arun_root": arun_root}
...
def test_declining_something_you_have_is_refused(sut, people):
    assert "already have" in sut.setup.decline("U01ARUN", "calc_root")


def test_an_unknown_item_is_refused(sut, people):
    assert "not something to set up" in sut.setup.decline("U0PI", "sudo")


def test_a_decline_can_be_undone(sut, people):
    sut.setup.decline("U0PI", "workflow")
...
# Nudging
# --------------------------------------------------------------------------- #
def test_only_one_thing_is_asked_at_a_time(sut, people):
    lines = sut.setup.nudge_lines("U0PI", first_turn=True)
    assert len(lines) == 1


def test_nothing_is_asked_mid_thread(sut, people):
    assert sut.setup.nudge_lines("U0PI", first_turn=False) == []


def test_a_declined_item_is_never_raised_again(sut, people):
    sut.setup.decline("U0PI", "workflow")
    sut.setup.decline("U0PI", "calc_root")
    assert sut.setup.nudge_lines("U0PI", first_turn=True) == []


def test_someone_with_both_is_left_alone(sut, people):
    sut.workflows.write("U01ARUN", "---\ndescription: d\n---\n\nbody\n")
    assert sut.setup.nudge_lines("U01ARUN", first_turn=True) == []


def test_the_root_nudge_explains_it_needs_an_admin(sut, people):
...
def test_the_preamble_carries_the_nudge(sut, people):
...
# --------------------------------------------------------------------------- #
def test_a_rootless_user_is_not_handed_the_shared_default(sut, people):
...
def test_a_rootless_user_can_still_read_everyone(sut, people):
    sut.setup.decline("U0PI", "calc_root")
    path, scope, err = sut.roots.resolve(".", "arun", "U0PI")
    assert not err and path == people["arun_root"] and scope["name"] == "arun"


def test_the_tools_report_that_clearly(sut, people):
    sut.setup.decline("U0PI", "calc_root")
    out = sut.dispatch("list_directory", {"path": "."}, _ctx("U0PI"))
    assert "no calculations of their own" in out


def test_declining_does_not_affect_anyone_else(sut, people):
    sut.setup.decline("U0PI", "calc_root")
...
# Requesting a root
# --------------------------------------------------------------------------- #
def test_requesting_a_root_files_it_and_tells_the_admin(sut, people, monkeypatch):
    monkeypatch.setattr(sut, "ADMIN_OVERRIDE", "U0SAM")
    ctx = _ctx("U0PI")
    new_root = people["tmp"] / "pi-calcs"
    new_root.mkdir()
...
def test_requesting_grants_nothing(sut, people):
    new_root = people["tmp"] / "pi-calcs2"
    new_root.mkdir()
...
def test_an_unusable_path_is_still_filed_but_flagged(sut, people):
...
def test_a_request_without_a_path_still_registers_interest(sut, people):
    out = sut.dispatch("request_calc_root", {}, _ctx("U0PI"))
    assert "let" in out and "know" in out
    assert sut.pending.find("calc_root", "U0PI") is not None


def test_a_request_is_always_for_the_speaker(sut, people):
...
def test_declining_a_root_withdraws_a_pending_request(sut, people):
...
def test_decline_setup_is_for_the_speaker_only(sut, people):
