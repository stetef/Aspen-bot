...
(C12–C16, spec §19–§20), now exercised against the real cluster, plus the security work still owed (notably the move to a
...
| C11 | **State-location guard at startup** | C8/C9 hold only if the registry and workflow tree aren't reachable through a writable path. Startup refuses if either resolves inside `WORKSPACE_ROOT` or `ASPEN_SANDBOX_WRITE_PATHS`, where sandboxed analysis code could edit them directly; the state dir is `chmod 0700` against other users on the shared node. Extended to the **job ledger**, the **template library** and the **runner library** — a writable ledger is a forgeable cancel, and a writable template or runner is content that later reaches a compute node. **Job staging is deliberately exempt** and world-readable: it holds a run's outputs, which are the point of submitting it and were unreadable to everyone including Aspen in the first build. The narrower rule that replaced it is what was doing the real work — staging is refused inside `ASPEN_SANDBOX_WRITE_PATHS` or the jail's own `figures/`/`cache/` binds, which are the only surfaces generated code can write. That same rule is what lets `results.resolve` read a batch's output back to the agent: because nothing the agent can *run* can write in staging, what it reads there is a run's output and not content the model planted — and the reader re-checks the rule itself rather than assuming the startup guard ran. | `main.py`, `config.py`, `results.py` |
...
- **A user can get code of their choosing to run as the shared account, by saving it
  as their own runner.** That is the intended workflow and the operator's explicit
  choice: an admin on that path was friction without benefit for a group they would
  trust with this account anyway. The reader is now the author, prompted by Aspen with
  the specific lines that turn dangerous on a shared account.
...
- **A batch whose IDs were never captured used to be untrackable.** The first real
  ...
  listed or cancelled through Aspen while consuming real compute. `jobs.backfill()` now
  recovers the IDs from the pipeline's own log on every ledger read, so the failure is
  self-healing rather than permanent. The residual is a submission whose IDs reach neither
  Aspen *nor* the pipeline's log, which would still be invisible.
...
- **Compute is spent on a shared allocation with per-user accounting kept only by
  ...
  its own association. The *full* reconcile is still manual (`aspen-users jobs
  reconcile`), not a cron, so consumption figures for old jobs are as fresh as the last
  time someone ran it — day to day the notify watcher and `list_my_jobs` call
  `jobs.refresh_states`, which is squeue-gated and rate-limited so that neither a poll
  loop nor a model in a loop can hammer slurmdbd. Bounded by the caps in C12; watch it
  during the beta rather than trusting it.
